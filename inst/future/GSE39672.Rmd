...
```{r, extractphenotypes}
samples <- gsub(".* ", "", pData(gset[[1]])$"title")
...
snp5.pop <- as.data.frame(mcols(hm450.manifest.pop.GoNL)[,grepl("MASK.snp5.*$", colnames(mcols(hm450.manifest.pop.GoNL)))])

gr <- hm450.manifest.pop.GoNL[rowSums(snp5.pop, na.rm=TRUE) == ncol(snp5.pop)]
mcols(gr) <- NULL
...
dnaCalls <- bplapply(1:22, getSnpMatrix, samples = samples, which=gr)
