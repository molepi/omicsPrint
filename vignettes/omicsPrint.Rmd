...
package on experimental data, i.e. 450k DNA methylation and genotype data.
...
reflects the same individual. However, swapping 50% of the SNPs is
...
or related individuals sample relationship verification can be run
between these two.
...
with the column names of `x` and `y`).
...
By setting the option `plot.it=FALSE` all data points are
returned. Thise can be used for creating customized plots of the IBS
mean and variance.

```{r, inferrelations2, fig.cap = "Scatter-plot of IBS mean and variance customized. Sometimes it can be convient draw the plot yourself for adding addtional information, for example, sample-pairs."}
data <- alleleSharing(x, y, relations=relations)
predicted <- inferRelations(data, plot.it=FALSE)
pchs <- paste(predicted$colnames.x, predicted$colnames.y, sep="-")
plot(predicted$mean, predicted$var, type="n")
text(predicted$mean, predicted$var, pchs)
```
