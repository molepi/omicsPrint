...
The following show the usage of this package using real world data.
Here we willshow how you could varify sample relationships on 
puplicly available DNA methylation data on a whole genome sequencing
dataset. The dataset used here contains pairs of monozygotic twins.
We will extract the beta-value matrix from GEO (GSE100940). There is 
currently no paper available for this dataset.

First lets start by cleaning up the workspace:
```{r clearworkspace}
rm(list=ls())
```
...
r$relation_type[r$idx == r$idy] <- "indentical"
head(r)
```
...
data <- alleleSharing(dnamCalls, relations = r, verbose = F)
mismatches <- inferRelations(data)
mismatches
```
