...
file <- tempfile()
download.file("ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE39nnn/GSE39672/matrix/GSE39672_series_matrix.txt.gz", file)
gset <- getGEO(filename=file) 
gset
se <- makeSummarizedExperimentFromExpressionSet(gset)
colnames(se) <- gsub("cell line: ", "", colData(se)$characteristics_ch1.2)
```


```{r, extractphenotypes}
population <- gsub("^.* ", "", colData(se)$characteristics_ch1)
table(population)
```
...
```{r, genotyping}
dnamCalls <- beta2genotype(se[cpgs,], assayName = "exprs")
dim(dnamCalls)
dnamCalls[1:5, 1:5]
save(dnamCalls, file="dnamCalls.RData")
load("dnamCalls.RData")
```
...
Since the samples used in this study (GSE39672) are overlapping with
...
gr
##check one
##samples <- gsub("cell line: ", "", colData(se)$characteristics_ch1.2)
##dnaCalls <- getSnpMatrix(22, samples = samples, which=gr)
##dim(dnaCalls)
library(BiocParallel)
register(MulticoreParam(2))
##dnaCalls <- bplapply(1:22, getSnpMatrix, samples = colnames(se), which=gr)
##dnaCalls <- bplapply(1:6, getSnpMatrix, samples = NULL, which=gr)
##dnaCalls <- do.call('rbind', dnaCalls)
##save(dnaCalls, file="dnaCallsRaw.RData")
load("dnaCallsRaw.RData")
dim(dnaCalls)
dnaCalls[1:5, 1:5]
```
...
enotype data using information for the array provider available from
the
...
mid <- match(map, rownames(se))
rownames(se)[mid[!is.na(mid)]] <- names(map)[!is.na(mid)]

library(omicsPrint)
dnamCalls <- beta2genotype(se[rownames(se) %in% rownames(dnaCalls),], assayName = "exprs")
...
with(plotdata, {
    plot(mean, var, pch=18, col=colors,
         xlab="mean (IBS)", ylab="variance (IBS)")
    points(2, 0, pch=18, col="black")
    legend("topright", levels(labels), col=levels(colors), pch=18, bty="n")
     })
